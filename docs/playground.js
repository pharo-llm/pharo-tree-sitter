...
function runQuery() {
  ...
  if (!q) {
    els.queryOut.textContent = "Enter a query.";
    return;
  }
  try {
    ...
    for (const match of cursor) {
      ...
      for (const capture of match.captures) {
        lines.push(
          `  @${capture.name} -> ${capture.node.type} [${capture.node.startPosition.row}:${capture.node.startPosition.column} -> ${capture.node.endPosition.row}:${capture.node.endPosition.column}]`
        );
      }
    }
